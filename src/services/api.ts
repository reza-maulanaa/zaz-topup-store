...
export const authApi = {
  ...
  login: (email: string, password: string) =>
    request<{ success: boolean; user: { id: string; name: string; email: string; role: string } }>(
      "/auth/login",
  ...
  deleteAccount: () =>
    request<{ success: boolean; message: string }>("/auth/delete-account", { method: "DELETE" }),
};
...
export const adminApi = {
  ...
  users: () =>
    request<{
      success: boolean;
      users: Array<{ id: string; name: string; email: string; role: string; createdAt: string; updatedAt: string }>;
    }>("/admin/users"),
